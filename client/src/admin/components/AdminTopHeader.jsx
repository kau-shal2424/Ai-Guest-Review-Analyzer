  ...
  return (
    <header className="sticky top-0 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 h-16 flex items-center px-4 sm:px-6 gap-4">
      ...
      <nav className="hidden sm:flex items-center gap-1.5 text-sm flex-1 min-w-0">
        <span className="text-slate-400 dark:text-slate-500 font-medium">Control Panel</span>
        {crumbs.map((crumb, i) => (
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div className="relative">
          <button
            ...
            className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-sm font-bold shadow-md hover:shadow-lg hover:shadow-violet-500/25 transition-all"
            ...
            <>
              ...
              >
                <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                  <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{user?.fullName || 'Admin'}</p>
