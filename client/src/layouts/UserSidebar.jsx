...
const NAV_ITEMS = [
  ...
  { to: '/user/analyze', icon: Sparkles, label: 'Analyze Reviews' },
  { to: '/user/reviews', icon: BookOpen, label: 'My Reviews' },
  { to: '/user/insights', icon: Brain, label: 'AI Insights' },
  { to: '/user/reports', icon: BarChart3, label: 'Reports' },
];

const BOTTOM_ITEMS = [
  { to: '/user/notifications', icon: Bell, label: 'Notifications', showBadge: true },
  { to: '/user/settings', icon: Settings, label: 'Settings' },
  { to: '/user/profile', icon: User, label: 'Profile' },
  { to: '/user/help', icon: HelpCircle, label: 'Help Center' },
];
...
export default function UserSidebar({ mobileOpen, onMobileClose, onCollapsedChange }) {
  const { user } = useAuth();
    ...
    <div className="flex flex-col h-full text-left">
      ...
      {!collapsed && user && (
        <div className="px-3 py-3 border-t border-[#EBEBEB] dark:border-[#333333]">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-[#717171] text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div>
        ...
        <p className="text-[10px] text-[#717171] font-medium">Guest Intelligence</p>
      </div>
    </div>
  );
}
