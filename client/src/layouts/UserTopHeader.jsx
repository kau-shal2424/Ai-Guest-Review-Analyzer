...
export default function UserTopHeader({ onMobileMenuOpen, sidebarCollapsed }) {
  ...
  return (
    <header className="sticky top-0 z-20 bg-white dark:bg-[#1a1a1a] border-b border-[#EBEBEB] dark:border-[#333333] h-16 flex items-center px-4 sm:px-6 gap-4">
      ...
      <nav className="hidden sm:flex items-center gap-1.5 text-xs font-semibold flex-1 min-w-0">
        <span className="text-[#717171]">Dashboard</span>
        {crumbs.map((crumb, i) => (
      ...
      {/* Airbnb Search Pill Hint */}
      <div 
        onClick={() => navigate('/user/analyze')}
        className="hidden md:flex items-center gap-2 px-3 py-1.5 border border-[#EBEBEB] dark:border-[#333333] rounded-full text-xs text-[#717171] hover:shadow-xs transition-shadow cursor-pointer bg-[#F7F7F7] dark:bg-[#222222]"
      >
        <Search className="w-3.5 h-3.5 text-[#FF385C]" />
        <span className="font-medium text-[#222222] dark:text-white">Analyze Reviews | AI Search</span>
      </div>
            ...
            <>
              ...
              >
                <div className="px-4 py-3 border-b border-[#EBEBEB] dark:border-[#333333]">
                  <p className="text-xs font-bold text-[#222222] dark:text-white truncate">{user?.fullName || 'User'}</p>
